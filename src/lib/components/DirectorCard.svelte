	...
	{#if !isCardFlipped}
		<div class="flex flex-col gap-y-4 items-center bg-surface-800 pt-8 px-6 h-full">
			<Avatar
				...
				width="w-[96px]"
				border="border-success-300 border-2"
			/>
			<div class="flex flex-col">
				<p class="text-lg text-success-500">{name}</p>
				<p class="text-sm italic">{role}</p>
			</div>
			<button
				class="rounded-full bg-error-500 w-fit p-5 absolute -bottom-7 hover:bg-success-500"
				on:click={() => (isCardFlipped = true)}
			>
		...
		<div class="flex flex-col gap-y-4 items-center bg-surface-800 pt-8 px-6 h-full">
			...
			<div class="flex gap-x-5">
				<svg
					...
					width="24"
					height="20"
					class="fill-white hover:fill-error-500 hover:cursor-pointer"
					><path
			...
			<button
				class="rounded-full bg-success-500 w-fit p-5 absolute -bottom-7 hover:bg-error-500"
				on:click={() => (isCardFlipped = false)}
			>
